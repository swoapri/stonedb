...
class VirtualColumnBase : public core::Column {
 public:
  struct VarMap {
    ...
    core::VarID var;  // variable identified by table alias and column number
    int col_ndx;      // column index (not negative)
    int dim;          // dimension in multiindex corresponding to the alias
    core::JustATable *tabp;
    std::shared_ptr<core::JustATable> GetTabPtr() const { return tab.lock(); }
    bool operator<(VarMap const &v) const {
      return (var < v.var) || ((var == v.var) && (col_ndx < v.col_ndx)) ||
             ((var == v.var) && (col_ndx == v.col_ndx) && (GetTabPtr() < v.GetTabPtr())) ||
             ((var == v.var) && (col_ndx == v.col_ndx) && (GetTabPtr() == v.GetTabPtr()) && (dim < v.dim));
    }
    std::weak_ptr<core::JustATable> tab;  // table corresponding to the alias
  };
   ...
   *
   */
  virtual const core::MysqlExpression::SetOfVars &GetParams() const { return params; }
  ...
  core::Transaction *ConnInfo() { return conn_info; }
  core::MultiIndex *GetMultiIndex() { return mind; }
 ...
 protected:
  ...
  core::Transaction *conn_info;
  std::vector<VarMap> var_map;
  core::MysqlExpression::SetOfVars params;  // parameters - columns from tables not
                                            // present in the local multiindex

  mutable std::shared_ptr<core::ValueOrNull> last_val;
  //! for caching, the first time evaluation is obligatory e.g. because a
  //! parameter has been changed

  mutable bool first_eval;

  int dim;  // valid only for SingleColumn and for ExpressionColumn if based on
            // a single table only
  // then this an easily accessible copy of var_map[0]._dim. Otherwise it should
  // be -1

  //////////// Local statistics (set by history) ///////////////
  int64_t vc_min_val;  // int64_t for ints/dec, *(double*)& for floats
  int64_t vc_max_val;
  bool vc_nulls_possible;  // false => no nulls, except of outer join ones (to
                           // be checked in multiindex)
  int64_t vc_dist_vals;    // upper approximation of non-null distinct vals, or
